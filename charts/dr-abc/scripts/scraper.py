...
while True:
    containerJSON = json.loads(http.request(
        'GET', 'https://kubernetes.default.svc.cluster.local/api/v1/namespaces/{}/pods'.format(namespace), headers=authHeader).data.decode('utf-8'))
        ...
        try:
            for thisState in container['status']['containerStatuses']:
                if 'waiting' in thisState['state']:
                    logging.warning('{} is waitning in state {}'.format(
                        containerName, thisState['state']['waiting']['reason']))
                    if thisState['state']['waiting']['reason'] == 'CrashLoopBackOff':
                        logging.warning(
                            'Deleting {} : CrashLoopBackOff'.format(containerName))
                        deleted = http.request('DELETE', 'https://kubernetes.default.svc.cluster.local/api/v1/namespaces/{}/pods/{}'.format(
                            namespace, containerName), headers=authHeader)
        except:
